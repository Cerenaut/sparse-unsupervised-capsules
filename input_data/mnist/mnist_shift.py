...
tf.flags.DEFINE_float('test_size', 0.14285,
                      'The proportion of the dataset to use for test set.')
tf.flags.DEFINE_boolean('shuffle', False, 'Shuffle the train/test datasets.')
tf.flags.DEFINE_boolean('download', False, 'Download the dataset files.')

MNIST_FILES = {
    'train': ('train-images-idx3-ubyte.gz', 'train-labels-idx1-ubyte.gz'),
    'valid_train': ('train-images-idx3-ubyte.gz',
...
def download_mnist():
    train_images, train_labels = MNIST_FILES['train']
    test_images, test_labels = MNIST_FILES['test']

    _download(train_images)
    _download(train_labels)
    _download(test_images)
    _download(test_labels)
...
def shift_write_mnist(dataset, filename, shift, pad, shuffle=False):
...
def sharded_writers(size, max_shard, num_images, file_prefix):
    ...
    Returns:
      ...
      writer_turns: A 2D matrix of size [num_images, ?] with the associated
        writer index for each example.
    """
...
def shift_write_multi_mnist(input_dataset, file_prefix, shift, pad, max_shard,
    ...
    Args:
      input_dataset: A list of tuples containing corresponding images and
        labels.
      file_prefix: String, prefix of the name of the resultant sharded tfrecord
        file.
      shift: Integer, the shift range for images.
    ...
    for i, (base_image, base_label) in enumerate(dataset):
        base_shifted = shift_2d(base_image, random_shifts[i, 0, :],
...
def read_byte_data(data_dir, split, tuples=True, keepdims=False):
    ...
    In summary: header size for image files is 4 * 4 bytes and for label file
    is 2 * 4 bytes.
    ...
    Args:
      ...
      split: String, the dataset split to process. It can be one of train,
        test, valid_train, valid_test.
    Returns:
      A list of (image, label). Image is a 28x28 numpy array and label is an
      int.
    """
...
def read_shuffled_data(data_dir):
    # Get training set
    train_images, train_labels = read_byte_data(data_dir, split='train',
                                                tuples=False, keepdims=True)

    # Get test set
    test_images, test_labels = read_byte_data(data_dir, split='test',
                                              tuples=False, keepdims=True)

    # Combine sets
    images = np.vstack([train_images, test_images])
    labels = np.vstack([train_labels, test_labels]).ravel()

    X_train, X_test, y_train, y_test = train_test_split(
        images, labels, test_size=FLAGS.test_size,
        random_state=FLAGS.seed, shuffle=True)
...
def main(_):
    ...
    if FLAGS.shuffle:
        train_data, test_data = read_shuffled_data(FLAGS.data_dir)

        file_format = 'shuffled_' + file_format
        train_shifted_filename = os.path.join(
            FLAGS.data_dir, file_format.format('train', 2))
        train_filename = os.path.join(
            FLAGS.data_dir, file_format.format('train', 0))
        test_filename = os.path.join(
            FLAGS.data_dir, file_format.format('test', 0))

        shift_write_mnist(train_data, train_shifted_filename, 2, 0)
        shift_write_mnist(train_data, train_filename, 0, 0)
        shift_write_mnist(test_data, test_filename, 0, 0)
    else:
        data = read_byte_data(FLAGS.data_dir, FLAGS.split)

        if FLAGS.multi_targets:
            file_format = 'multi_' + file_format
