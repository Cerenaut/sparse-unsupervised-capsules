...
Handles reading from Cifar10 dataset saved in binary original format. Scales
and normalizes the images as the preprocessing step. It can distort the images
by random cropping and contrast adjusting.
"""
...
def _distort_resize(image, image_size):
    ...
    Adds standard distortions such as flipping, cropping and changing
...
def _batch_features(image, label, batch_size, split, image_size, evaluate):
    """Constructs the batched feature dictionary.

    Batches the images and labels accourding to the split. Shuffles the data
    only if split is train. Formats the feature dictionary to be in the format
    required by experiment.py.
...
def inputs(split, data_dir, batch_size, evaluate=False):
    ...
    if split == 'train':
        filenames = [
            os.path.join(data_dir,
                         'data_batch_%d.bin' % i) for i in range(1, 6)
        ]
    else:
    ...
    return _batch_features(
        image, label, batch_size, split, image_size, evaluate)
