...
class CapsuleModelTest(tf.test.TestCase):
    ...
    def testBuildCapsule(self):
        ...
        The output shape should be [batch, num_latent_capsules, 16]. Also each
        capsule layer should declare 2 sets of variables (weight and bias),
        therefore single call to _build_capsule declares 4 variables for a
        total of 2 capsule layers.
        """
        with tf.Graph().as_default():
            test_model = capsule_model.CapsuleModel(self.hparams)
            toy_input = np.reshape(
                np.arange(256 * 14 * 14), (1, 1, 256, 14, 14))
            ...
            self.assertListEqual([self.hparams.num_latent_capsules, 16],
                                 [capsules.value, atoms.value])
    ...
    def testIntegrity(self):
        ...
        multiple times without any increase in number of trainable variables
    ...
    def testInferenceWithRemake(self):
        ...
        should declare 6 sets of variables (weight and bias) and different
        targets should share the variables.
        """
        with tf.Graph().as_default():
